...
char ssid[] = "BigTree"; // your network SSID (name)
char pass[] = "xuyihan1994"; // your network password
...
unsigned int receivePiAddressPort = 2920; //
// master variables
...
void setup()
{
    Serial.begin(115200);   // this is for sliding window
    Serial.println();
    Serial.println();
    Serial.println("");
    Serial.println("--------------------------");
...
void greenlightChange(uint16_t clearColor){
  uint16_t scaled = clearColor * 255 / 65535;
  if(scaled > 255) scaled = 255;
  analogWrite(D5, scaled);
}
...
{   
    ...
    //else {
        ...
        if (packetBuffer[1] == LIGHT_UPDATE_PACKET)
        {
            ...
            if (packetBuffer[3] == 0){ Serial.print("SLAVE"); }
...
{
    int sendToLightSwarm = 15;
    Serial.print("Broadcast ToSwarm = ");
    Serial.print(sendToLightSwarm);
    Serial.print(" ");

    IPAddress sendSwarmAddress(172, 20, 10, sendToLightSwarm); // mySwarm Address
...
void checkAndSetIfMaster()
{
    ...
    for (i = 0; i < SWARMSIZE; i++)
    {
        ...
        if (swarmTimeStamp[i] == 0){ Serial.print("TO "); }
        
        else if (swarmTimeStamp[i] == -1){ Serial.print("NP "); }
        
        else if (swarmTimeStamp[i] == 1){ Serial.print("ME "); }
...
void sendLogToServer(){
    ...
    if (masterState == true)
    {
        ...
        if ((serverAddress[0] == 0) && (serverAddress[1] == 0)){ return; }    // we are done. not defined
        else
        {
            ...
            for (i = 0; i < SWARMSIZE; i++)
            {
                 ...
                 else if (swarmTimeStamp[i] == -1){ strcat(stateString, "NP"); }
            
                 else if (swarmTimeStamp[i] == 1){ strcat(stateString, "PR"); }
           
                 else{ strcat(stateString, "PR"); }
       ...
       packetBuffer[4] = lux; // Software Version
       ...
       int packetLength;
       packetLength = i + 5 + 1;
       udp.beginPacket(serverAddress, receivePiAddressPort); //
