...
// local port to listen for UDP
// True if master, False if not

IPAddress serverAddress = IPAddress(0, 0, 0, 0); // default no IP
...
Serial.print(" K - ");
  ...
  if (!cb) {
    ...
    udp.read(packetBuffer, PACKET_SIZE); // read the packet into the
buffer
...
void broadcastARandomUpdatePacket()
{
  ...
  Serial.print("ms : ");
  delay(randomDelay);
  IPAddress sendSwarmAddress(192, 168, 1, sendToLightSwarm); // my
Swarm Address
...
| ....next Swarm ID
    ...
    packetBuffer[2] = localIP[3];     // Sending Swarm Number
    packetBuffer[3] = strlen(myBuildString); // length of string in
bytes
    packetBuffer[4] = VERSIONNUMBER;  // Software Version
    int i;
